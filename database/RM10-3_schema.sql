...
--
-- Table structure for table `o_openmeeting`
--

CREATE TABLE `o_openmeeting` (
  `id` int(11) NOT NULL COMMENT 'id for open meeting'
) ENGINE=MyISAM DEFAULT CHARSET=latin1 COLLATE=latin1_general_ci;
...
--
-- Table structure for table `to_event`
--

CREATE TABLE `to_event` (
  `id` int(11) NOT NULL COMMENT 'open event record id',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'date/time created',
...
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='Holds core information for an open event';
...
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='Course definitions';
...
CREATE TABLE `t_coursedetail` (
  ...
  `upddate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'date of last update',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'date record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='Definition for each fleet';
...
CREATE TABLE `t_cruise` (
  ...
  `chg-helm` varchar(40) COLLATE latin1_general_ci DEFAULT NULL COMMENT 'has value if helm name is changed',
  `chg-crew` varchar(40) COLLATE latin1_general_ci DEFAULT NULL COMMENT 'temporary crew change for races today',
  `chg-hullcolour` varchar(20) COLLATE latin1_general_ci DEFAULT NULL,
  `chg-sailnum` varchar(20) COLLATE latin1_general_ci DEFAULT NULL COMMENT 'temporary sailno change for today',
  ...
  `vhfchannel` varchar(10) COLLATE latin1_general_ci DEFAULT NULL,
...
CREATE TABLE `t_event` (
  ...
  `series_code` varchar(100) COLLATE latin1_general_ci DEFAULT NULL COMMENT 'text code to link to series definition in t_series - code includes specific year here but not in t_series (e.g. FROSTBITE-24)',
  `series_code_extra` varchar(100) CHARACTER SET utf8 DEFAULT NULL COMMENT 'additional series code links if event is part of more than one series - comma separated list',
...
--
-- Stand-in structure for view `v_competitor`
-- (See below for the actual view)
--
CREATE TABLE `v_competitor` (
`id` int(10)
,`classId` int(10)
,`boatNumber` varchar(20)
,`sailNumber` varchar(20)
,`hullColour` varchar(20)
,`helm` varchar(40)
,`crew` varchar(40)
,`active` tinyint(1)
,`club` varchar(60)
...
--
-- Stand-in structure for view `v_cruise`
-- (See below for the actual view)
--
CREATE TABLE `v_cruise` (
`id` int(11)
,`leisureSailType` varchar(20)
,`leisureSailDate` date
,`timeRegistered` time
,`timeSignedBackIn` time
,`boatId` int(11)
,`action` enum('register','update','declare')
,`hullColour` varchar(20)
,`helm` varchar(40)
,`crewNumber` varchar(20)
,`vhfChannel` varchar(10)
,`telno` varchar(20)
,`clubVhf` tinyint(4)
...
--
-- Stand-in structure for view `v_tide`
-- (See below for the actual view)
--
CREATE TABLE `v_tide` (
`id` int(11)
,`theDate` date
,`highWater1Time` char(5)
,`highWater1Height` char(4)
,`highWater2Time` char(5)
,`highWater2Height` char(4)
,`heightUnits` char(3)
,`timeReference` char(6)
);
...
--
-- Table structure for table `z_entry_steamer`
--

CREATE TABLE `z_entry_steamer` (
  `id` int(11) NOT NULL COMMENT 'table primary key',
  `action` enum('enter','delete','update','retire','replace','declare') COLLATE latin1_general_ci NOT NULL COMMENT 'action for racemanager to take on entry',
  `protest` tinyint(1) NOT NULL DEFAULT '0' COMMENT '1 if declaring protest',
  `status` enum('N','L','F','E','A') COLLATE latin1_general_ci NOT NULL DEFAULT 'N' COMMENT 'status (N=not loaded, L=loaded, F=failed. E=not eligible, A=not allocated)',
...
--
-- Table structure for table `z_tide`
--
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_class`  AS  select `t_class`.`id` AS `id`,`t_class`.`classname` AS `className` from `t_class` ;

-- --------------------------------------------------------

--
-- Structure for view `v_competitor`
--
DROP TABLE IF EXISTS `v_competitor`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_competitor`  AS  select `t_competitor`.`id` AS `id`,`t_competitor`.`classid` AS `classId`,`t_competitor`.`boatnum` AS `boatNumber`,`t_competitor`.`sailnum` AS `sailNumber`,`t_competitor`.`hullcolour` AS `hullColour`,`t_competitor`.`helm` AS `helm`,`t_competitor`.`crew` AS `crew`,`t_competitor`.`active` AS `active`,`t_competitor`.`club` AS `club`,`t_competitor`.`upddate` AS `updateDateTime`,`t_competitor`.`updby` AS `updatedBy`,`t_competitor`.`createdate` AS `createDateTime` from `t_competitor` ;

-- --------------------------------------------------------

--
-- Structure for view `v_cruise`
--
DROP TABLE IF EXISTS `v_cruise`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_cruise`  AS  select `t_cruise`.`id` AS `id`,`t_cruise`.`cruise_type` AS `leisureSailType`,`t_cruise`.`cruise_date` AS `leisureSailDate`,`t_cruise`.`time_in` AS `timeRegistered`,`t_cruise`.`time_out` AS `timeSignedBackIn`,`t_cruise`.`boatid` AS `boatId`,`t_cruise`.`action` AS `action`,`t_cruise`.`chg-hullcolour` AS `hullColour`,`t_cruise`.`chg-helm` AS `helm`,`t_cruise`.`chg-numcrew` AS `crewNumber`,`t_cruise`.`vhfchannel` AS `vhfChannel`,`t_cruise`.`telno` AS `telno`,`t_cruise`.`clubvhf` AS `clubVhf`,`t_cruise`.`destination` AS `destination`,`t_cruise`.`createdate` AS `createDateTime`,`t_cruise`.`upddate` AS `updateDateTime`,`t_cruise`.`updby` AS `updatedBy` from `t_cruise` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_event`  AS  select `t_event`.`id` AS `id`,`t_event`.`event_date` AS `eventDate`,`t_event`.`event_start` AS `eventStart`,`t_event`.`event_name` AS `eventName`,`t_event`.`event_type` AS `eventType`,`t_event`.`event_status` AS `eventStatus`,`t_event`.`event_notes` AS `eventNotes` from `t_event` ;

-- --------------------------------------------------------

--
-- Structure for view `v_tide`
--
DROP TABLE IF EXISTS `v_tide`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_tide`  AS  select `t_tide`.`id` AS `id`,`t_tide`.`date` AS `theDate`,`t_tide`.`hw1_time` AS `highWater1Time`,`t_tide`.`hw1_height` AS `highWater1Height`,`t_tide`.`hw2_time` AS `highWater2Time`,`t_tide`.`hw2_height` AS `highWater2Height`,`t_tide`.`height_units` AS `heightUnits`,`t_tide`.`time_reference` AS `timeReference` from `t_tide` ;
...
--
-- Indexes for table `to_event`
--
ALTER TABLE `to_event`
  ADD PRIMARY KEY (`id`);
...
--
-- Indexes for table `z_entry_steamer`
--
ALTER TABLE `z_entry_steamer`
  ADD PRIMARY KEY (`id`);

--
-- Indexes for table `z_tide`
--
ALTER TABLE `z_tide`
  ADD PRIMARY KEY (`id`);
...
--
-- AUTO_INCREMENT for table `to_event`
--
ALTER TABLE `to_event`
...
--
-- AUTO_INCREMENT for table `z_entry_steamer`
--
ALTER TABLE `z_entry_steamer`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT COMMENT 'table primary key';

--
-- AUTO_INCREMENT for table `z_tide`
--
ALTER TABLE `z_tide`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
COMMIT;
