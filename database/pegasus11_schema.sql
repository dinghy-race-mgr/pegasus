...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 17, 2025 at 05:19 PM
...
CREATE TABLE `a_entry` (
  `id` int(11) NOT NULL,
  `action` enum('enter','delete','update','retire','replace','declare') NOT NULL COMMENT 'action for racemanager to take on entry',
  `protest` tinyint(1) NOT NULL DEFAULT '0' COMMENT '1 if declaring protest',
  `status` enum('N','L','F','E','A') NOT NULL DEFAULT 'N' COMMENT 'status (N=not loaded, L=loaded, F=failed. E=not eligible, A=not allocated)',
  ...
  `memberid` varchar(20) DEFAULT NULL COMMENT 'memberid from t_competitor',
  `chg-helm` varchar(40) DEFAULT NULL COMMENT 'has value if helm name is changed',
  `chg-crew` varchar(40) DEFAULT NULL COMMENT 'temporary crew change for races today',
  `chg-sailnum` varchar(20) DEFAULT NULL COMMENT 'temporary sailno change for today',
  ...
  `updby` varchar(20) DEFAULT NULL COMMENT 'last update by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='archive table used to store race entry information in the t_entry structure';
...
CREATE TABLE `a_lap` (
  ...
  `updby` varchar(20) DEFAULT NULL COMMENT 'last update by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='archive table used to store lap information in the t_lap structure';
...
CREATE TABLE `a_race` (
  ...
  `competitorid` int(11) NOT NULL COMMENT 'competitor record id from t_competitor',
  `trackerid` varchar(40) DEFAULT NULL COMMENT 'identifier for position tracker',
  `helm` varchar(40) NOT NULL DEFAULT '' COMMENT 'helm name',
  `crew` varchar(40) DEFAULT NULL COMMENT 'crew name',
  `club` varchar(60) DEFAULT NULL COMMENT 'club name',
  `class` varchar(40) NOT NULL DEFAULT '' COMMENT 'boat class name',
  `classcode` varchar(6) DEFAULT NULL COMMENT 'class acronym',
  `sailnum` varchar(20) NOT NULL DEFAULT '' COMMENT 'sail number used in race',
  ...
  `code` varchar(10) DEFAULT NULL COMMENT 'result code for competitor (e.g. DNF)',
  ...
  `declaration` enum('D','R','X') NOT NULL DEFAULT 'X' COMMENT 'declaration status [X - not declared, D declared, R retired]',
  `protest` tinyint(1) NOT NULL DEFAULT '0' COMMENT 'protest flag',
  `note` varchar(200) DEFAULT NULL COMMENT 'notes field',
  `status` enum('R','F','X','D') DEFAULT NULL COMMENT 'status field (R = racing, F = finished, X = excluded, D = deleted',
  `upddate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'last update date',
  `updby` varchar(20) DEFAULT NULL COMMENT 'last update by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='archive table used to store race information in the t_race structure';
...
CREATE TABLE `e_form` (
  `id` int(11) NOT NULL COMMENT 'id for form recod',
...
CREATE TABLE `t_cfgfleet` (
  ...
  `fleet_code` varchar(10) NOT NULL COMMENT 'short code for fleet (e.g. SH)',
  `fleet_name` varchar(60) NOT NULL COMMENT 'full name for fleet (e.g. slow handicap)',
  `fleet_desc` varchar(200) DEFAULT NULL COMMENT 'description of fleet',
  `scoring` enum('handicap','average','level','pursuit') NOT NULL COMMENT 'type of race scoring (should this be integer code)',
  `py_type` enum('national','local','personal') NOT NULL DEFAULT 'local' COMMENT 'type of PN number to be used for this race',
  `warn_signal` varchar(20) DEFAULT NULL COMMENT 'filename for signal flag for warning signal',
  `prep_signal` varchar(20) DEFAULT NULL COMMENT 'filename for signal flag used for preparatory signal',
  ...
  `classinc` varchar(200) DEFAULT NULL COMMENT 'list of classes to be allocated to this race',
  `onlyinc` int(1) NOT NULL DEFAULT '0' COMMENT 'if set - only the listed included classes are allowed',
  `classexc` varchar(200) DEFAULT NULL COMMENT 'list of classes to be excluded from this race',
  `groupinc` varchar(200) DEFAULT NULL COMMENT 'groups to be eligible for this race',
  `min_py` int(4) DEFAULT '0' COMMENT 'minimum PY for this race',
  `max_py` int(4) DEFAULT '2000' COMMENT 'maximum PY for this race',
  `crew` varchar(40) DEFAULT NULL COMMENT 'crew constraint for classes in this race',
  `spintype` varchar(40) DEFAULT NULL COMMENT 'spinnaker type constraints for classes in this race',
  `hulltype` varchar(40) DEFAULT NULL COMMENT 'hull type constraint for classes in this race',
  `min_helmage` char(3) DEFAULT NULL COMMENT 'minimum helm of age for inclusion in this race',
  `max_helmage` char(3) DEFAULT NULL COMMENT 'maximum age of helm for this race',
  `min_skill` char(2) DEFAULT NULL COMMENT 'minimum skill level for this race',
  `max_skill` char(2) DEFAULT NULL COMMENT 'maximum skill level for this race',
  `upddate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'date of last update',
  `updby` varchar(20) DEFAULT NULL COMMENT 'last update by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='defines racing fleets for each race format';
...
CREATE TABLE `t_cfgrace` (
  `id` int(6) NOT NULL,
  `race_code` varchar(6) NOT NULL,
  `race_name` varchar(30) NOT NULL,
  `race_desc` varchar(500) DEFAULT NULL,
  `pursuit` tinyint(1) NOT NULL DEFAULT '0' COMMENT 'true if a pursuit race format',
  `numstarts` tinyint(3) NOT NULL,
  `start_scheme` varchar(20) NOT NULL COMMENT 'start scheme for preparatory, warning and start signals (e.g 5,4,1,0 or 10,5,0)',
  ...
  `updby` varchar(20) DEFAULT NULL COMMENT 'last update by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='defines event configuration in terms of individual races for racing events (e.g club series).';
...
CREATE TABLE `t_cfgseries` (
  ...
  `seriestype` varchar(30) NOT NULL COMMENT 'series type code (e.g. short, long etc)',
  `discard` varchar(60) NOT NULL COMMENT 'discard profile usig Sailwave coding (e.g. 0,0,1,1,2 means in the five race series one discard is applied after three races and 2 discards if all five races are complete)',
  `nodiscard` varchar(40) DEFAULT NULL COMMENT 'if set last race sailed cannot be discarded',
  `multiplier` varchar(40) DEFAULT NULL COMMENT 'if set the score for the last race in series is multiplied by the value ',
  `avgscheme` varchar(40) NOT NULL COMMENT 'scheme for calculating average points',
  `dutypoints` varchar(20) NOT NULL COMMENT 'duty points',
  `maxduty` int(2) NOT NULL DEFAULT '10' COMMENT 'max number of duties allowed to score',
  `notes` varchar(500) DEFAULT NULL COMMENT 'Notes on series - e.g. a description',
...
CREATE TABLE `t_class` (
  ...
  `acronym` char(6) NOT NULL DEFAULT 'xxx' COMMENT '3 letter acronym for class (e.g. FB -> fireball)',
  `classname` varchar(40) NOT NULL COMMENT 'class name',
  `variant` varchar(40) DEFAULT NULL COMMENT 'name for class recognised variant',
  `info` varchar(255) DEFAULT NULL COMMENT 'description',
  ...
  `rya_id` char(40) DEFAULT NULL COMMENT 'rya unique identifier',
  `upd_year` year(4) DEFAULT NULL COMMENT 'year of last update',
  `category` enum('D','M','K','C','F','P','W') NOT NULL DEFAULT 'D' COMMENT 'boat category [D=dinghy, M=multihull, K=keelboat, F=foiler, P = paddled boat]',
  `crew` enum('1','2','N') NOT NULL DEFAULT '1' COMMENT 'number of crew',
  `rig` enum('U','S','K','N') NOT NULL DEFAULT 'U' COMMENT 'rig type [U=una, S=sloop, K=two masts]',
  `spinnaker` enum('O','C','A') NOT NULL DEFAULT 'O' COMMENT 'spinnaker type [C=conventional, A=asymmetric, O=none]',
  `engine` enum('OB','IBF','IB2','IB3') NOT NULL DEFAULT 'OB' COMMENT 'engine [OB=outboard, IB2=2 bladed fixed propellor, IB3=3 bladed fixed propellor, IBF=folding propellor]',
  `keel` enum('D','F','2K','3K','N') NOT NULL DEFAULT 'D' COMMENT 'keel type [D=drop keel, F=single fixed, 2K=twin keel, 3K=triple keel]',
  `fleets` varchar(200) DEFAULT NULL COMMENT 'comma separated list of fleet categories (e.g. gold, silver, etc.)	',
  `active` tinyint(1) NOT NULL DEFAULT '1' COMMENT 'record active flag',
  `upddate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'last update date',
  `updby` varchar(20) DEFAULT NULL COMMENT 'last update by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='class configuration details - includes RYA coding and yardstick information';
...
CREATE TABLE `t_code_result` (
  ...
  `code` char(6) NOT NULL COMMENT 'result code (e.g. DNF)',
  `short` varchar(60) NOT NULL COMMENT 'short description',
  `info` varchar(500) NOT NULL COMMENT 'description',
  `scoringtype` enum('penalty','race','series','manual') NOT NULL COMMENT 'code to determine when scoring can be applied',
  `scoring` varchar(20) NOT NULL COMMENT 'scoring equation (using N= num in race, S=num in series, )',
  ...
  `listorder` int(2) NOT NULL DEFAULT '1' COMMENT 'order for display',
  ...
  `updby` varchar(40) NOT NULL,
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='list of codes used in result processing (e.g. DNF) and information on how they should be scored';
...
CREATE TABLE `t_code_system` (
  `id` int(11) NOT NULL COMMENT 'table primary key',
  `groupname` varchar(40) NOT NULL COMMENT 'code group',
  `code` varchar(40) NOT NULL COMMENT 'code value',
  `label` varchar(200) NOT NULL COMMENT 'code label (for use in interface)',
  `listorder` int(3) NOT NULL DEFAULT '0' COMMENT 'order in list',
  ...
  `upddate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'date of last update',
  `updby` varchar(20) DEFAULT NULL COMMENT 'updated by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='codes used in the racemanager applications';
...
CREATE TABLE `t_code_type` (
  ...
  `groupname` varchar(40) NOT NULL COMMENT 'name for code which is used to identify the codes belonging to this type',
  `label` varchar(40) NOT NULL COMMENT 'label for the code type',
  `info` varchar(500) DEFAULT NULL COMMENT 'description for the code type',
  `listorder` int(3) NOT NULL DEFAULT '1' COMMENT 'display order',
  `type` enum('system','club') NOT NULL,
  `upddate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'last update date',
  `updby` varchar(20) DEFAULT NULL COMMENT 'last update by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='types of codes used in racemanager applications';
...
CREATE TABLE `t_competitor` (
  ...
  `boatnum` varchar(20) NOT NULL COMMENT 'boat number',
  `sailnum` varchar(20) NOT NULL COMMENT 'sail number used',
  `boatname` varchar(60) DEFAULT NULL COMMENT 'name (or sponsor) of boat',
  `hullcolour` varchar(20) DEFAULT NULL COMMENT 'colour of hull',
  `helm` varchar(40) NOT NULL COMMENT 'helm name',
  `helm_dob` date DEFAULT NULL COMMENT 'helm date of birth',
  `helm_email` varchar(100) DEFAULT NULL COMMENT 'helm email address',
  `telephone` varchar(20) DEFAULT NULL COMMENT 'contact telephone',
  `crew` varchar(40) DEFAULT NULL COMMENT 'crew name',
  `crew_dob` date DEFAULT NULL COMMENT 'crew date of birth',
  `crew_email` varchar(100) DEFAULT NULL COMMENT 'crew email address',
  `club` varchar(60) DEFAULT NULL COMMENT 'sailing club name',
  `personal_py` int(5) DEFAULT NULL COMMENT 'personal PN',
  `skill_level` varchar(20) DEFAULT NULL COMMENT 'skill coding',
  `flight` varchar(20) DEFAULT NULL COMMENT 'allocated flight for specific events',
  ...
  `prizelist` varchar(200) DEFAULT NULL COMMENT 'comma seperated prize eligibility',
  `grouplist` varchar(100) DEFAULT NULL COMMENT 'comma separated list of member groups (e.g. member, visitor, junior)',
  `memberid` varchar(20) DEFAULT NULL COMMENT 'member id/code',
  `trackerid` varchar(40) DEFAULT NULL COMMENT 'identifier for position tracker',
  `upddate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'last update date',
  `updby` varchar(20) DEFAULT NULL COMMENT 'last update by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='competitor details';
...
CREATE TABLE `t_cruise` (
  ...
  `action` enum('register','update','declare') NOT NULL COMMENT 'action for racemanager to take on entry',
  `chg-helm` varchar(40) DEFAULT NULL COMMENT 'has value if helm name is changed',
  `chg-crew` varchar(40) DEFAULT NULL COMMENT 'temporary crew change for races today',
  `chg-hullcolour` varchar(20) DEFAULT NULL,
  `chg-sailnum` varchar(20) DEFAULT NULL COMMENT 'temporary sailno change for today',
  `chg-numcrew` varchar(20) DEFAULT NULL COMMENT 'total number of people on boad',
  `chg-contact` varchar(40) DEFAULT NULL COMMENT 'contact details - typically mobile number',
  `vhfchannel` varchar(10) DEFAULT NULL,
  ...
  `updby` varchar(20) DEFAULT NULL COMMENT 'last update by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='information on logged leisure sailing';
...
CREATE TABLE `t_entry` (
  ...
  `updby` varchar(20) DEFAULT NULL COMMENT 'last update by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='information on race entries';
...
CREATE TABLE `t_event` (
  ...
  `event_start` varchar(40) DEFAULT NULL COMMENT 'time of start (as text HH:MM)',
  `event_order` int(2) DEFAULT '1' COMMENT 'order of event on day - used if start times not defined',
  `event_name` varchar(60) NOT NULL COMMENT 'event name (e.g. Summer Series, Kathleen Trophy, etc.)',
  `series_code` varchar(100) DEFAULT NULL COMMENT 'foreign key to series information in t_series',
  `series_code_extra` varchar(100) DEFAULT NULL COMMENT 'additional series code links if event is part of more than one series - comma separated list',
  `event_type` varchar(20) NOT NULL COMMENT 'type of event (e.g. cruise, race, etc.)',
  `event_format` int(11) DEFAULT NULL COMMENT 'race format for event - references event configuration record in t_eventcfg',
  `event_entry` varchar(20) DEFAULT NULL COMMENT 'entry type (e.g ood or electronic signon etc.) - codes in t_code',
  `event_status` varchar(40) DEFAULT 'scheduled' COMMENT 'current status of event - codes in t_code',
  `event_open` enum('club','open') NOT NULL DEFAULT 'club' COMMENT 'indicates if it is an open event',
  `event_ood` varchar(40) DEFAULT NULL COMMENT 'name of ood (alternate overrides name in t_eventduty)',
  `tide_time` varchar(10) DEFAULT NULL COMMENT 'time of high water (as text HH:MM)',
  `tide_height` varchar(10) DEFAULT NULL COMMENT 'height of tide in metres',
  `start_scheme` varchar(20) DEFAULT NULL COMMENT 'field to change the default start scheme specified in event configuration',
  ...
  `ws_start` varchar(20) DEFAULT NULL COMMENT 'wind speed at start of race (from coded range)',
  `wd_start` varchar(20) DEFAULT NULL COMMENT 'wind direction at start (from coded range)',
  `ws_end` varchar(20) DEFAULT NULL COMMENT 'wind speed at end of race (from coded range)',
  `wd_end` varchar(20) DEFAULT NULL COMMENT 'wind direction at end (from coded range)',
  `event_notes` varchar(500) DEFAULT NULL COMMENT 'notes field will appear in programme display',
  `result_notes` varchar(500) DEFAULT NULL COMMENT 'notes relating to the results',
  ...
  `weblink` varchar(200) DEFAULT NULL COMMENT 'url to web page with more info',
  `webname` varchar(40) DEFAULT NULL COMMENT 'text to use on url link',
  `display_code` char(10) DEFAULT 'W,R,S' COMMENT 'code to define whether displayed on website, racemanager, and/or sailor - comma separated list W,R,S',
  `active` tinyint(1) NOT NULL DEFAULT '0' COMMENT 'true if part of published programme',
  `upddate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'last update date',
  `updby` char(20) DEFAULT NULL COMMENT 'last update by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='programmed events for racemanager';
...
CREATE TABLE `t_eventduty` (
  ...
  `updby` varchar(20) DEFAULT NULL COMMENT 'last update by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='information allocated duties for each programmed event';
...
CREATE TABLE `t_help` (
  `id` int(11) NOT NULL COMMENT 'table primary key',
  `category` varchar(60) NOT NULL COMMENT 'help category - comma separated',
  `question` varchar(500) NOT NULL COMMENT 'question or help item heading',
  `answer` varchar(4000) NOT NULL COMMENT 'answer text',
  `notes` varchar(4000) DEFAULT NULL COMMENT 'additional related information',
  `author` varchar(50) DEFAULT NULL COMMENT 'person creating help item',
  `pursuit` tinyint(1) NOT NULL DEFAULT '0' COMMENT 'if set should only be displayed if it is a pursuit race',
  `eventname` varchar(60) DEFAULT NULL COMMENT 'event name constraint - can be name substring',
  ...
  `listorder` int(3) NOT NULL DEFAULT '1' COMMENT 'order in which they should be displayed within category',
  `active` tinyint(1) NOT NULL DEFAULT '1' COMMENT 'if set to 1 can be viewed by user',
  `upddate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'date of last update',
  `updby` varchar(20) NOT NULL COMMENT 'user responsible for last update',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='Holds help information which is configurable for each installation';
...
CREATE TABLE `t_ini` (
  `id` int(11) NOT NULL COMMENT 'table primary key',
  `category` varchar(40) NOT NULL COMMENT 'configuration parameter category (club, interface, entry, results, admin]',
  `parameter` varchar(40) NOT NULL COMMENT 'name of parameter',
  `label` varchar(40) NOT NULL,
  `value` varchar(200) NOT NULL COMMENT 'value of parameter',
  `notes` varchar(500) DEFAULT NULL COMMENT 'Information on what values can be used for this setting',
  `upddate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'date of last update',
  `updby` char(20) DEFAULT NULL COMMENT 'last update by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='club specific configurable parameters used by racemanager';
...
CREATE TABLE `t_lap` (
  ...
  `updby` varchar(20) DEFAULT NULL COMMENT 'last update by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='information on each lap recorded';
...
CREATE TABLE `t_link` (
  `id` int(11) NOT NULL COMMENT 'table primary key',
  `label` varchar(60) NOT NULL COMMENT 'label for link label to appear in interface',
  `url_link` varchar(200) NOT NULL COMMENT 'URL for link (e.g. http://....)',
  `tip` varchar(500) DEFAULT NULL COMMENT 'tooltip text for link',
  `category` varchar(20) NOT NULL COMMENT 'link category (allows grouping of links for different pages in system) - only one category permitted so it may be necessary to duplicate links if they are to appear in more than one place.',
  `listorder` tinyint(2) NOT NULL DEFAULT '0' COMMENT 'order in which to display links (within category)',
  `upddate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'last update date',
  `updby` varchar(20) DEFAULT NULL COMMENT 'last update by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='Club specific links to additional information';
...
CREATE TABLE `t_message` (
  ...
  `name` varchar(60) NOT NULL COMMENT 'person / process creating message',
  `subject` varchar(60) NOT NULL COMMENT 'message subject',
  `message` text NOT NULL COMMENT 'message content',
  `email` varchar(100) DEFAULT NULL COMMENT 'email address for response',
  `response` text COMMENT 'action taken',
  `status` enum('received','closed') NOT NULL DEFAULT 'received' COMMENT 'current status of message',
  `upddate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `updby` varchar(20) NOT NULL COMMENT 'user responsible for update',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='messages created by user (or actions of user) in racemanager applications for review by system admin';
...
CREATE TABLE `t_protest` (
  `id` int(11) NOT NULL,
  `protest_type` varchar(20) NOT NULL,
  `event_name` varchar(60) NOT NULL,
  `event_date` date NOT NULL,
  `protestor_name` varchar(60) NOT NULL,
  `protestor_class` varchar(40) NOT NULL,
  `protestor_sailnum` varchar(20) NOT NULL,
  `protestee_name` varchar(60) DEFAULT NULL,
  `protestee_class` varchar(40) DEFAULT NULL,
  `protestee_sailnum` varchar(20) DEFAULT NULL,
  `infotxt` varchar(4000) NOT NULL,
  `facts_found` varchar(4000) DEFAULT NULL,
  `decision` varchar(2000) DEFAULT NULL,
  `submitted` datetime NOT NULL,
  `upddate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `updby` varchar(20) DEFAULT NULL,
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='information on submitted protests';
...
CREATE TABLE `t_race` (
  ...
  `updby` varchar(20) DEFAULT NULL COMMENT 'last update by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='current state for each competitor in races today';
...
CREATE TABLE `t_racestate` (
  ...
  `start` int(2) NOT NULL COMMENT 'start number',
  `racetype` enum('handicap','average','level','pursuit','flight') NOT NULL COMMENT 'type of race (pursuit, fleet, handicap, etc.) - should this be integer code',
  ...
  `status` enum('notstarted','inprogress','finishing','allfinished') DEFAULT NULL COMMENT 'current race status',
  `prevstatus` varchar(20) DEFAULT NULL COMMENT 'previous status',
  `upddate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'last update date',
  `updby` varchar(20) DEFAULT NULL COMMENT 'last update by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='summary race status information for each race/fleet';
...
CREATE TABLE `t_result` (
  ...
  `race_type` enum('handicap','average','level','pursuit','none') NOT NULL COMMENT 'race type as defined in t_racecfg (stored here to indicate scoring mechanism used when race was run)',
  `competitorid` int(11) NOT NULL COMMENT 'competitor record id (from t_competitor)',
  `class` varchar(40) NOT NULL COMMENT 'class of boat (text)',
  `sailnum` varchar(20) NOT NULL COMMENT 'sail number used',
  `pn` int(4) NOT NULL COMMENT 'PN used for this race',
  `apn` int(4) DEFAULT NULL COMMENT 'achieved PN (personal handicap)',
  `helm` varchar(40) NOT NULL COMMENT 'helm name for this race',
  `crew` varchar(40) DEFAULT NULL COMMENT 'crew name for this race',
  `club` varchar(60) DEFAULT NULL COMMENT 'club name',
  `lap` int(2) NOT NULL,
  `etime` int(6) NOT NULL COMMENT 'elapsed time at finish (secs)',
  `ctime` int(6) NOT NULL DEFAULT '0' COMMENT 'corrected time at finish',
  `atime` int(6) NOT NULL DEFAULT '0' COMMENT 'aggregate time (pro-rata time for all boats doing the same number of laps in average lap race)',
  `code` char(6) DEFAULT NULL COMMENT 'finish code (e.g. DNC, DNF)',
  `penalty` decimal(4,1) NOT NULL DEFAULT '0.0',
  `points` decimal(4,1) NOT NULL COMMENT 'points awarded for race',
  `declaration` char(3) NOT NULL COMMENT 'declared - same codes as in t_race',
  `note` varchar(200) DEFAULT NULL COMMENT 'notes',
  `upddate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'last update date',
  `updby` varchar(20) DEFAULT NULL COMMENT 'last updated by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='result information for each boat in each event';
...
CREATE TABLE `t_resultfile` (
  ...
  `folder` enum('races','series','class','special') NOT NULL COMMENT 'type of result file (reflects structure of results directory)',
  `format` enum('htm','csv','pdf','jpg') NOT NULL COMMENT 'data format of file',
  `filename` varchar(80) NOT NULL COMMENT 'name of file',
  `label` varchar(60) DEFAULT NULL COMMENT 'label for presenting the results file to users',
  `notes` varchar(500) DEFAULT NULL COMMENT 'supporting notes describing the file',
  `status` enum('final','provisional','embargoed') NOT NULL DEFAULT 'final' COMMENT 'status for result file',
  ...
  `updby` varchar(20) DEFAULT NULL COMMENT 'last update by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='information on results files created by racemanager or imported';
...
CREATE TABLE `t_rotamember` (
  `id` int(11) NOT NULL,
  `memberid` varchar(40) DEFAULT NULL COMMENT 'id for this member in membership system',
  `firstname` varchar(20) NOT NULL COMMENT 'first name',
  `familyname` varchar(40) NOT NULL COMMENT 'famil nmae',
  `rota` varchar(40) DEFAULT NULL COMMENT 'rota name',
  `phone` varchar(20) DEFAULT NULL COMMENT 'phone number (mobile preferred)',
  `email` varchar(60) DEFAULT NULL COMMENT 'email',
  `note` varchar(500) DEFAULT NULL COMMENT 'rota notes (e.g. not on wednesdays)',
  `partner` varchar(40) DEFAULT NULL COMMENT 'name of duty partner',
  `active` tinyint(1) NOT NULL DEFAULT '1' COMMENT 'set to 1 if still active',
  `updby` varchar(20) NOT NULL COMMENT 'last update by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created',
  `upddate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'date of last update'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='Information on who is on each duty rota';
...
CREATE TABLE `t_series` (
  `id` int(11) NOT NULL COMMENT 'table primary key',
  `seriescode` varchar(40) NOT NULL COMMENT 'e.g.  AUTUMN',
  `seriesname` varchar(50) NOT NULL COMMENT 'name of series (e.g. Autumn Series)',
  `seriestype` int(11) NOT NULL COMMENT 'series type from t_cfgseries',
  `startdate` varchar(6) DEFAULT NULL COMMENT 'earliest date on which series race can be sailed (e.g 1-jan)',
  `enddate` varchar(6) DEFAULT NULL COMMENT 'latest date on which series race can be sailed (e.g 30-sep)',
  `race_format` int(11) DEFAULT NULL COMMENT 'default race format (id in t_cfgrace)',
  `merge` varchar(100) DEFAULT NULL COMMENT 'lists of classes to be merged in series result -  each class separated by ,,|,,',
  `classresults` varchar(300) DEFAULT NULL COMMENT 'holds comma separated list of classes for which class specific results are required',
  `opt_style` varchar(20) NOT NULL DEFAULT 'classic' COMMENT 'report style to be used',
  ...
  `opt_racelabel` enum('number','dates') NOT NULL DEFAULT 'dates' COMMENT 'defines what is used to label each race in the series (number or dates)',
...
CREATE TABLE `t_tide` (
  ...
  `hw1_time` char(5) DEFAULT NULL COMMENT 'first high water time (hh:mm)',
  `hw1_height` char(4) NOT NULL COMMENT 'first high water height (e.g 3.9)',
  `hw2_time` char(5) DEFAULT NULL COMMENT 'second high water time (hh:mm)',
  `hw2_height` char(4) DEFAULT NULL COMMENT 'second high water height',
  `time_reference` char(6) NOT NULL DEFAULT 'local',
  `height_units` char(3) NOT NULL DEFAULT 'm',
  `upddate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'last update date',
  `updby` varchar(20) DEFAULT NULL COMMENT 'last update by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='holds tide data';
...
CREATE TABLE `z_entry` (
  ...
  `updby` varchar(20) DEFAULT NULL COMMENT 'last update by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='A temporary copy of t_entry used to hold entries for the demo events';
...
CREATE TABLE `z_entry_draft` (
  ...
  `updby` varchar(20) DEFAULT NULL COMMENT 'last update by',
  `createdate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'timestamp when record created'
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='temporary table to receive event entries from rm_event (e_entry)';
