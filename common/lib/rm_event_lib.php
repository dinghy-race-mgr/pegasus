...
{
    /*
        class list is  defined by a RM race format stored in the entry-classes field.  This supports a) all classes known to RM, b) specified classes
        For all classes we use a datalist to select - but that means we have to accept whatever they enter

        Typical problems we are getting that prevent finding a racemanager class
            - random blanks at beginning or end mean that we are not getting a match  'Blaze ' will not be found  (FIXED - have added trim to string)
            - wrong structure  e.g. 'RS 300' rather than 'RS300', 'GP14' rather than 'GP 14' (possible fix - search on individual words)
            - adding extra words e.g. 'Comet Trio Mk1' rather than 'Comet Trio'    (possible fix  - use better class names)
            - not completing rig e.g. 'RS Aero' rather than 'RS Aero 7'            (possible fix could check all hits and pick one)
    */

    // could try search on individual words
...
function validate_entries()
{
    ...
    foreach ($entries as $k => $entry) {
        ...
        $num_juniors = 0;
        if (!empty($entry['h-age']) and $entry['h-age'] < 18) {
            $num_juniors++;
        }
        if (!empty($entry['c-age']) and $entry['c-age'] < 18) {
            $num_juniors++;
        }
        if ($num_juniors > 0) {
        ...
        if ($num_juniors > 0)  // check if we have consents
        {
            ...
            if ($num_consents < $num_juniors) {
                $entries[$k]['chk2'] = "missing consents";
            };
        }

        // check 3
        $entries[$k]['chk3'] = false;
        if (empty($entry['h-emergency']) and empty($entry['c-emergency'])) {
            $entries[$k]['chk3'] = "missing emergency contact";
        }

        // check 4
        $entries[$k]['chk4'] = false;
        if ($doublehander and (empty($entry['c-name']) or strtolower($entry['c-name'] == "tbc"
                    or strtolower($entry['c-name']) == "tba" or strtolower($entry['c-name']) == "tbd"))) {
            $entries[$k]['chk4'] = "missing crew name";
        }

        // check 5
        $entries[$k]['chk5'] = false;
        if ($entry['h-gender'] == 'not given' or ($doublehander and $entry['c-gender'] == 'not given')) {
            $entries[$k]['chk5'] = "missing gender";
        }

        // check 6
        $entries[$k]['chk6'] = false;
        if (empty($entry['b-sailno']) or strtolower($entry['b-sailno']) == "tbc"
            or strtolower($entry['b-sailno']) == "tba" or strtolower($entry['b-sailno']) == "tbd") {
            $entries[$k]['chk6'] = "missing sail number";
        }
