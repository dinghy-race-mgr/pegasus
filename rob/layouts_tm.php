...
function page ($params = array())
{
    ...
    <head>
        ...
        <!-- Bootstrap core CSS -->
        <link href="../common/oss/bootstrap532/css/cerulean_bootstrap.min.css" rel="stylesheet">
...
function class_table($params = array())
{
    $htm = "";

    $table = "";
    foreach($params['data'] as $k => $row){
        $table.= "<tr><td>{$row['classname']}</td><td>{$row['nat_py']}</td><td>{$row['spec']}</td></tr>";
    }

    $htm.= <<<EOT
    <table class="table table-hover table-striped">
        <thead><tr><th>Class</th><th>Yardstick</th><th>Classification</th></tr></thead>
        <tbody>
            $table
        </tbody>
    </table>
